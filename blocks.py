...
def model_load(checkpoint, ckpt_dir, model, optim_E, optim_S, optim_G):
    if not os.path.exists(ckpt_dir):
        ...
        epoch = -1
        tot_itr=0
        return model, optim_E, optim_S, optim_G, epoch, tot_itr

    ckpt_path = Path(ckpt_dir)
    if checkpoint:
        model_ckpt = ckpt_path + '/' + checkpoint
    else:
        ckpt_lst = list(ckpt_path.glob('model_iter_*'))  # Convert generator to list
        if len(ckpt_lst) == 0:
            ...
            # Sort the checkpoints by iteration or epoch
            ckpt_lst.sort(key=lambda x: int(x.name.split('iter_')[1].split('_epoch')[0]))
            model_ckpt = ckpt_lst[-1]

    itr = int(model_ckpt.name.split('iter_')[1].split('_epoch_')[0])
    epoch = int(model_ckpt.name.split('iter_')[1].split('_epoch_')[1].split('.')[0])
...
def test_model_load(checkpoint, model):
    dict_model = torch.load(checkpoint, map_location=torch.device('cpu'), weights_only=True)
